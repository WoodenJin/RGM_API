...
def trajectory_produce(h0, period, step):
    ...
    leg_left_hip = -leg_angle[:, 0]
    leg_left_knee = leg_angle[:, 1]
    leg_left_ankle = -(leg_angle[:, 0] + leg_angle[:, 1])
    leg_right_hip = leg_angle[:, 0]
    leg_right_knee = -leg_angle[:, 1]
    leg_right_ankle = (leg_angle[:, 0] + leg_angle[:, 1])
    angle_control = np.stack((leg_left_hip, leg_left_knee, leg_left_ankle,
                              leg_right_hip, leg_right_knee, leg_right_ankle), axis=-1)
    speed_control = []
    for i in range(angle_control.shape[0] - 1):
        speed_control.append((angle_control[i + 1, :] - angle_control[i, :]) / step)
    # print(leg_angle)
    # print(angle_control.shape)pyth
