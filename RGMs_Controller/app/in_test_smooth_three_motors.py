...
temp = angle_path[:, 0] + angle_path[:, 1]
# print(temp.shape)
temp = np.reshape(temp, (100, 1))
angle_path = np.hstack((angle_path, temp))
print(angle_path)
...
id = [1, 2, 3]  # the motor's id in the CAN network
...
angle_init = [165, 190, 177]  # get current angle
...
for i in range(N):
    angle_control.append(
        [angle_init[0] + angle_path[i][2], angle_init[1] + angle_path[i][1], angle_init[2] + angle_path[i][0]])
